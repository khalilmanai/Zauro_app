          ...
          <div className="flex items-center space-x-4">
            ...
            {balance && (
              <div className="hidden sm:flex items-center space-x-2 text-sm bg-white/15 px-3 py-1.5 rounded-lg border border-white/20">
                ...
                <span className="text-white font-medium">
                  {Number.parseFloat(balance.hbar).toFixed(2)} HBAR
                </span>
              </div>
            )}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="relative h-8 w-8 rounded-full text-white/60 hover:text-white hover:bg-white/10 p-0"
                >
                  <Avatar className="h-8 w-8">
                    <AvatarFallback className="bg-[#093102] text-white font-semibold text-xs">
                      {user.firstName?.[0] ?? ""}
                      {user.lastName?.[0] ?? ""}
                    </AvatarFallback>
                  </Avatar>
              ...
              <DropdownMenuContent
                className="w-56 bg-white/10 backdrop-blur-xl border-white/20 text-white"
                align="end"
                forceMount
              >
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    ...
                    {user.role && (
                      <Badge className="w-fit text-xs mt-1 bg-[#0288D1]/20 text-[#0288D1] border border-[#0288D1]/40">
                        {user.role.replace("_", " ")}
                      </Badge>
                    )}
                  </div>
                ...
                <DropdownMenuItem asChild>
                  <Link href="/profile" className="flex items-center text-white hover:bg-white/15 cursor-pointer">
                ...
                <DropdownMenuItem asChild>
                  <Link href="/settings" className="flex items-center text-white hover:bg-white/15 cursor-pointer">
                ...
                <DropdownMenuItem
                  ...
                  className="text-red-100 hover:bg-red-500/20 cursor-pointer"
                >
